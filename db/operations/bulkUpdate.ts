...
export function bulkUpdate<U extends object>(input: IBulkUpdateInput<U>): SQLWrapper {
  ...
  for (const key of updatableKeys) {
    query.append(sql`, ${sql.raw(key as string)}`);
  }

  query.append(sql`) AS (VALUES `);

  for (const [index, update] of input.updates.entries()) {
    query.append(sql`(${update[idColumnName]}`);

    for (const key of updatableKeys) {
      query.append(sql`, ${update[key]}`);
    }

    query.append(index === input.updates.length - 1 ? sql`)` : sql`), `);
  }

  query.append(sql`) UPDATE ${input.idColumn.table} SET `);

  query.append(sql.join(updatableKeys.map((name) => {
    const nameSql = sql.raw(name as string);
    return sql`${nameSql} = updates.${nameSql}`;
  }), sql`, `));

  query.append(sql` FROM updates WHERE ${idColumnSql} = updates.id`);

  if (input.where) {
    query.append(sql` AND (${input.where})`);
  }
